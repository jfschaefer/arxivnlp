    ...
    def test_tokenize(self):
        ...
        for s in sentences[::-1]:
            ...
            for word in word_tokenize(s)[::-1]:
                doc.insert_node(node=etree.XML('<span style="color:orange; font-size:120%">]</span>'),pos=word.backrefs[-1]+1)
                doc.insert_node(node=etree.XML('<span style="color:orange; font-size:120%">[</span>'), pos=word.backrefs[0])
