...
def sentence_tokenize(substring: SubString) -> List[SubString]:
    ...
    for i in range(len(substring)):
        ...
        if in_header and not is_in_header(substring.get_node(i)):
            in_header = False
            new_sent_start = i
        if is_display_math(substring.get_node(i)) and not is_display_math(substring.get_node(i+1)) and substring.string[i+1].isspace() and substring.string[i+2].upper():
            new_sent_start = i + 1
...
def normal_end_of_sentence(substring: SubString, i: int) -> bool:
    ...
    if isdot and i + 1 < len(substring) and substring.string[i + 1].islower():
        return False
    if isdot and 0 < i < len(substring) - 1 and substring.string[i - 1].isdigit() and substring.string[
        i + 1].isdigit():
        return False
